...
import { VIDEOS_LIST_TOGGLE_LIKE } from "@/config/api-path";
...
const FavoriteButton = ({ video_id, like_id, setIsLiked = () => {} }) => {
  ...
  const toggleLike = (e, video_id) => {
    e.preventDefault()
    fetch(`${VIDEOS_LIST_TOGGLE_LIKE}/${video_id}`, {
        method: "GET",
        headers: {
          ...getAuthHeader()}
        })
  ...
  return (
    <button
      onClick={(e) => toggleLike(e, video_id)}
