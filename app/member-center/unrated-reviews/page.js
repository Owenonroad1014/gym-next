      ...
      didOpen: () => {
        ...
        for (let i = 1; i <= 5; i++) {
          ...
          starSpan.onclick = () => {
            currentRating = i
            document
              .querySelectorAll('#star-container span')
              .forEach((s, index) => {
                const starIcon =
                  index < i
          ...
          const initialIcon = React.createElement(FaRegStar, {
            color: '#f87808',
            size: 36,
          })
      ...
      <div className={styles.sort}>
        <Link
          ...
          className={`${styles.sortItem} ${
            pathname === '/member-center/rated-reviews' ? styles.active : ''
          }`}
        >
          已評價
        </Link>
        <Link
          ...
          className={`${styles.sortItem} ${
            pathname === '/member-center/unrated-reviews' ? styles.active : ''
          }`}
        >
          尚未評價
        </Link>
                ...
                <div className={styles.contentItems}>
                  ...
                  <div className={styles.contentArea}>
                    <div className={styles.contentItem}>
                      ...
                      {product.weight !== null && (
                        <div>商品規格: {product.weight}公斤</div>
                      )}
                      <div>
                        訂單日期:{' '}
                        {new Date(product.added_at).toLocaleString('zh-TW', {
                          hour12: false,
                        })}
                      </div>
                    </div>
